  ...
  const sendData = async (data) => {
    ...
    try {
      const response = await fetch('https://verionx-api.onrender.com/api/v1/users/register-mail-to-mail-list', {
        ...
        body: JSON.stringify({ data: data }), // Convert data to JSON format
      });
      ...
      if (response.ok) {
        ...
        setIsPending(false);
        // Handle successful data sending (e.g., clear input field, display success message)
      } else {
    ...
    <div className='bg-f5 heroGeneral d-flex gap-4 flex-column flex-xl-row justify-content-center justify-content-between align-items-center'>
        <div style={{maxWidth:"700px"}} className=' textcontainer mb-5 mx-xxl-1 px-xxl-0 d-flex flex-column container '>
            ...
            <form className="d-flex">
                <input type="text" value={userInput} onChange={handleInputChange} className=' container p-3 mb-xl-5 inputText' placeholder="Search for any service..."  style={{boxShadow: "-1px 0px 5px 0px rgba(0, 0, 0, 0.2)"}} />
                ...
                {isPending && <button disabled className="bg-eb p-3 mb-xl-5 txt-ff inputButton" style={{boxShadow: "0px -1px 5px 0px rgba(0, 0, 0, 0.2)"}}>.....</button>}
            </form>
