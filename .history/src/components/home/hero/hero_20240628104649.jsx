  ...
  async function handleSubmit(event) {
    ...
    try {
      const response = await fetch('https://verionx-api.onrender.com/api/v1/users/register-mail-to-mail-list', {
        // Add any necessary request options here (e.g., method, body)
      });
  
      if (response.ok) {
        const data = await response.json();
        // Handle successful response and fetched data
      } else {
        throw new Error(`Failed to fetch data: ${response.status}`);
      }
    } catch (error) {
      console.error('Error:', error);
      // Handle fetch errors here, e.g., display an error message to the user
    }
  }
    ...
    <div className='bg-f5 heroGeneral d-flex gap-4 flex-column flex-xl-row justify-content-center justify-content-between align-items-center'>
        <div style={{maxWidth:"700px"}} className=' textcontainer mb-5 mx-xxl-1 px-xxl-0 d-flex flex-column container '>
            ...
            <form className="d-flex">
                <input type="text" name='subscribe' className=' container p-3 mb-xl-5 inputText' placeholder="Search for any service..."  style={{boxShadow: "-1px 0px 5px 0px rgba(0, 0, 0, 0.2)"}} />
                {!isPending && <button className="bg-eb p-3 mb-xl-5 inputButton" onClick={handleSubmit} style={{boxShadow: "0px -1px 5px 0px rgba(0, 0, 0, 0.2)"}}><CustomSvg name="search"/></button>}
                {isPending && <button disabled className="bg-eb p-3 mb-xl-5 txt-ff inputButton" onClick={handleSubmit} style={{boxShadow: "0px -1px 5px 0px rgba(0, 0, 0, 0.2)"}}>.....</button>}
            </form>
